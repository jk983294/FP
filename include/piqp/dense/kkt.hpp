...
struct KKT {
    ...
    void update_kkt() {
        kkt_mat.template triangularView<Eigen::Lower>() =
            data.P_utri.transpose() + m_rho * Mat<T>::Identity(data.n, data.n);

        if (data.m > 0) {
            W_delta_inv_G = (m_z_inv.cwiseProduct(m_s) + Vec<T>::Constant(data.m, m_delta)).asDiagonal().inverse() *
                            data.GT.transpose();
            kkt_mat.template triangularView<Eigen::Lower>() += data.GT * W_delta_inv_G;
        }

        for (isize i = 0; i < data.n_lb; i++) {
            kkt_mat.diagonal()(data.x_lb_idx(i)) +=
                data.x_lb_scaling(i) * data.x_lb_scaling(i) / (m_z_lb_inv(i) * m_s_lb(i) + m_delta);
        }

        for (isize i = 0; i < data.n_ub; i++) {
            kkt_mat.diagonal()(data.x_ub_idx(i)) +=
    ...
    void solve(const CVecRef<T>& rhs_x, const CVecRef<T>& rhs_y, const CVecRef<T>& rhs_z, const CVecRef<T>& rhs_z_lb,
        ...
        delta_s_lb.head(data.n_lb).array() =
            m_z_lb_inv.head(data.n_lb).array() *
            (rhs_s_lb.head(data.n_lb).array() - m_s_lb.head(data.n_lb).array() * delta_z_lb.head(data.n_lb).array());

        delta_s_ub.head(data.n_ub).array() =
            m_z_ub_inv.head(data.n_ub).array() *
            (rhs_s_ub.head(data.n_ub).array() - m_s_ub.head(data.n_ub).array() * delta_z_ub.head(data.n_ub).array());
