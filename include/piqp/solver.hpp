...
enum SolverMatrixType { PIQP_DENSE = 0, PIQP_SPARSE = 1 };

template <typename Derived, typename T, typename I, typename Preconditioner, int MatrixType,
          int Mode = KKTMode::KKT_FULL>
class SolverBase {
protected:
...
public:
    ...
    Status solve() {
        if (m_settings.verbose) {
            ...
            piqp_print("\n");
            piqp_print(
                "iter  prim_obj       dual_obj       duality_gap   prim_inf      dual_inf      rho         delta       "
                "mu          p_step   d_step\n");
        }
...
protected:
    void setup_impl(const CMatRefType& P, const CVecRef<T>& c, const optional<CMatRefType>& A,
                    const optional<CVecRef<T>>& b, const optional<CMatRefType>& G, const optional<CVecRef<T>>& h,
                    const optional<CVecRef<T>>& x_lb, const optional<CVecRef<T>>& x_ub) {
        if (m_settings.compute_timings) {
            m_timer.start();
        }
        ...
        m_preconditioner.scale_data(m_data, false, m_settings.preconditioner_scale_cost,
    ...
    Status solve_impl() {
        ...
        if (!m_kkt_init_state) {
            ...
            m_kkt.update_scalings(m_result.info.rho, m_result.info.delta, m_result.s, m_result.s_lb, m_result.s_ub,
        ...
        rs_lb.setZero();
        rs_ub.setZero();
        m_kkt.solve(rx, m_data.b, m_data.h, m_data.x_lb_n, m_data.x_ub, rs, rs_lb, rs_ub, m_result.x, m_result.y,
                    m_result.z, m_result.z_lb, m_result.z_ub, m_result.s, m_result.s_lb, m_result.s_ub,
        ...
        if (m_data.m + m_data.n_lb + m_data.n_ub > 0) {
            ...
            T delta_s_bar = delta_s + (T(0.5) * tmp_prod) / (m_result.z.sum() + z_lb.sum() + z_ub.sum() +
                                                             T(m_data.m + m_data.n_lb + m_data.n_ub) * delta_z);
            T delta_z_bar = delta_z + (T(0.5) * tmp_prod) / (m_result.s.sum() + s_lb.sum() + s_ub.sum() +
                                                             T(m_data.m + m_data.n_lb + m_data.n_ub) * delta_s);
            ...
            m_result.info.mu = (m_result.s.dot(m_result.z) + s_lb.dot(z_lb) + s_ub.dot(z_ub)) /
                               T(m_data.m + m_data.n_lb + m_data.n_ub);
        }
        ...
        while (m_result.info.iter < m_settings.max_iter) {
            ...
            if (m_settings.verbose) {
                piqp_print("%3zd   % .5e   % .5e   %.5e   %.5e   %.5e   %.3e   %.3e   %.3e   %.4f   %.4f\n",
                           m_result.info.iter, (double)m_result.info.primal_obj, (double)m_result.info.dual_obj,
                           (double)m_result.info.duality_gap, (double)m_result.info.primal_inf,
                           (double)m_result.info.dual_inf, (double)m_result.info.rho, (double)m_result.info.delta,
                           (double)m_result.info.mu, (double)m_result.info.primal_step,
            ...
            if (m_result.info.primal_inf < m_settings.eps_abs + m_settings.eps_rel * m_result.info.primal_rel_inf &&
                ...
                (!m_settings.check_duality_gap ||
                 m_result.info.duality_gap <
                     m_settings.eps_duality_gap_abs + m_settings.eps_duality_gap_rel * m_result.info.duality_gap_rel)) {
            ...
            rz_lb.head(m_data.n_lb) =
                rz_lb_nr.head(m_data.n_lb) - m_result.info.delta * (nu_lb.head(m_data.n_lb) - z_lb.head(m_data.n_lb));
            rz_ub.head(m_data.n_ub) =
                rz_ub_nr.head(m_data.n_ub) - m_result.info.delta * (nu_ub.head(m_data.n_ub) - z_ub.head(m_data.n_ub));
            ...
            if (m_data.m + m_data.n_lb + m_data.n_ub > 0) {
                ...
                m_kkt.solve(rx, ry, rz, rz_lb, rz_ub, rs, rs_lb, rs_ub, dx, dy, dz, dz_lb, dz_ub, ds, ds_lb, ds_ub,
                            m_enable_iterative_refinement);
                ...
                m_result.info.sigma +=
                    (s_lb + alpha_s * ds_lb.head(m_data.n_lb)).dot(z_lb + alpha_z * dz_lb.head(m_data.n_lb));
                m_result.info.sigma +=
                    (s_ub + alpha_s * ds_ub.head(m_data.n_ub)).dot(z_ub + alpha_z * dz_ub.head(m_data.n_ub));
                ...
                rs_lb.head(m_data.n_lb).array() += -ds_lb.head(m_data.n_lb).array() * dz_lb.head(m_data.n_lb).array() +
                                                   m_result.info.sigma * m_result.info.mu;
                rs_ub.head(m_data.n_ub).array() += -ds_ub.head(m_data.n_ub).array() * dz_ub.head(m_data.n_ub).array() +
                                                   m_result.info.sigma * m_result.info.mu;
                ...
                if (dual_inf_nr() < 0.95 * m_result.info.dual_inf ||
                    (m_result.info.rho == m_settings.reg_finetune_lower_limit && dual_prox_inf() < 1e2)) {
                    m_result.zeta = m_result.x;
                    m_result.info.rho = std::max(m_result.info.reg_limit, (T(1) - mu_rate) * m_result.info.rho);
                } else {
                    m_result.info.no_primal_update++;
                    m_result.info.rho =
                        std::max(m_result.info.reg_limit, (T(1) - T(0.666) * mu_rate) * m_result.info.rho);
                }

                if (primal_inf_nr() < 0.95 * m_result.info.primal_inf ||
                    (m_result.info.delta == m_settings.reg_finetune_lower_limit && primal_prox_inf() < 1e2)) {
                    m_result.lambda = m_result.y;
                    m_result.nu = m_result.z;
                    nu_lb = z_lb;
                    nu_ub = z_ub;
                    ...
                    m_result.info.delta =
                        std::max(m_result.info.reg_limit, (T(1) - T(0.666) * mu_rate) * m_result.info.delta);
                }
            } else {
    ...
    void update_nr_residuals() {
        ...
        m_result.info.duality_gap_rel =
            std::max(m_result.info.duality_gap_rel, m_preconditioner.unscale_cost(abs(tmp)));
        ...
        m_result.info.dual_rel_inf = std::max(
            m_result.info.dual_rel_inf, m_preconditioner.unscale_dual_res(m_data.c).template lpNorm<Eigen::Infinity>());
        ...
        m_result.info.dual_rel_inf = std::max(m_result.info.dual_rel_inf,
                                              m_preconditioner.unscale_dual_res(dx).template lpNorm<Eigen::Infinity>());
        rx_nr.noalias() -= dx;
        ...
        m_result.info.primal_rel_inf =
            std::max(m_result.info.primal_rel_inf,
                     m_preconditioner.unscale_primal_res_eq(m_data.b).template lpNorm<Eigen::Infinity>());

        rz_nr.noalias() = -m_data.GT.transpose() * m_result.x;
        m_result.info.primal_rel_inf =
            std::max(m_result.info.primal_rel_inf,
                     m_preconditioner.unscale_primal_res_ineq(rz_nr).template lpNorm<Eigen::Infinity>());
        rz_nr.noalias() += m_data.h - m_result.s;
        m_result.info.primal_rel_inf =
            std::max(m_result.info.primal_rel_inf,
                     m_preconditioner.unscale_primal_res_ineq(m_data.h).template lpNorm<Eigen::Infinity>());
        m_result.info.primal_rel_inf =
            std::max(m_result.info.primal_rel_inf,
                     m_preconditioner.unscale_primal_res_ineq(m_result.s).template lpNorm<Eigen::Infinity>());

        for (isize i = 0; i < m_data.n_lb; i++) {
            rz_lb_nr(i) = m_data.x_lb_scaling(i) * m_result.x(m_data.x_lb_idx(i));
        }
        m_result.info.primal_rel_inf = std::max(
            m_result.info.primal_rel_inf,
            m_preconditioner.unscale_primal_res_lb(rz_lb_nr.head(m_data.n_lb)).template lpNorm<Eigen::Infinity>());
        rz_lb_nr.head(m_data.n_lb).noalias() += m_data.x_lb_n.head(m_data.n_lb) - m_result.s_lb.head(m_data.n_lb);
        m_result.info.primal_rel_inf = std::max(
            m_result.info.primal_rel_inf,
            m_preconditioner.unscale_primal_res_lb(m_data.x_lb_n.head(m_data.n_lb)).template lpNorm<Eigen::Infinity>());
        m_result.info.primal_rel_inf = std::max(
            m_result.info.primal_rel_inf,
            m_preconditioner.unscale_primal_res_lb(m_result.s_lb.head(m_data.n_lb)).template lpNorm<Eigen::Infinity>());
        ...
        m_result.info.primal_rel_inf = std::max(
            m_result.info.primal_rel_inf,
            m_preconditioner.unscale_primal_res_ub(rz_ub_nr.head(m_data.n_ub)).template lpNorm<Eigen::Infinity>());
        rz_ub_nr.head(m_data.n_ub).noalias() += m_data.x_ub.head(m_data.n_ub) - m_result.s_ub.head(m_data.n_ub);
        m_result.info.primal_rel_inf = std::max(
            m_result.info.primal_rel_inf,
            m_preconditioner.unscale_primal_res_ub(m_data.x_ub.head(m_data.n_ub)).template lpNorm<Eigen::Infinity>());
        m_result.info.primal_rel_inf = std::max(
            m_result.info.primal_rel_inf,
            m_preconditioner.unscale_primal_res_ub(m_result.s_ub.head(m_data.n_ub)).template lpNorm<Eigen::Infinity>());
    }
    ...
    T primal_inf_nr() {
        ...
        inf = std::max(
            inf, m_preconditioner.unscale_primal_res_lb(rz_lb_nr.head(m_data.n_lb)).template lpNorm<Eigen::Infinity>());
        inf = std::max(
            inf, m_preconditioner.unscale_primal_res_ub(rz_ub_nr.head(m_data.n_ub)).template lpNorm<Eigen::Infinity>());
        return inf;
    }
    ...
    T primal_inf_r() {
        ...
        inf = std::max(
            inf, m_preconditioner.unscale_primal_res_lb(rz_lb.head(m_data.n_lb)).template lpNorm<Eigen::Infinity>());
        inf = std::max(
            inf, m_preconditioner.unscale_primal_res_ub(rz_ub.head(m_data.n_ub)).template lpNorm<Eigen::Infinity>());
        return inf;
    }
    ...
    T primal_prox_inf() {
        ...
        inf = std::max(inf,
                       m_preconditioner.unscale_dual_ineq(m_result.nu - m_result.z).template lpNorm<Eigen::Infinity>());
        inf = std::max(
            inf, m_preconditioner.unscale_dual_lb(m_result.nu_lb.head(m_data.n_lb) - m_result.z_lb.head(m_data.n_lb))
                     .template lpNorm<Eigen::Infinity>());
        inf = std::max(
            inf, m_preconditioner.unscale_dual_ub(m_result.nu_ub.head(m_data.n_ub) - m_result.z_ub.head(m_data.n_ub))
                     .template lpNorm<Eigen::Infinity>());
        return inf;
    }

    T dual_inf_nr() { return m_preconditioner.unscale_dual_res(rx_nr).template lpNorm<Eigen::Infinity>(); }

    T dual_inf_r() { return m_preconditioner.unscale_dual_res(rx).template lpNorm<Eigen::Infinity>(); }
...
class DenseSolver
    : public SolverBase<DenseSolver<T, Preconditioner>, T, int, Preconditioner, PIQP_DENSE, KKTMode::KKT_FULL> {
public:
    void setup(const CMatRef<T>& P, const CVecRef<T>& c, const optional<CMatRef<T>>& A = nullopt,
               const optional<CVecRef<T>>& b = nullopt, const optional<CMatRef<T>>& G = nullopt,
               const optional<CVecRef<T>>& h = nullopt, const optional<CVecRef<T>>& x_lb = nullopt,
               const optional<CVecRef<T>>& x_ub = nullopt) {
        this->setup_impl(P, c, A, b, G, h, x_lb, x_ub);
    }

    void update(const optional<CMatRef<T>>& P = nullopt, const optional<CVecRef<T>>& c = nullopt,
                const optional<CMatRef<T>>& A = nullopt, const optional<CVecRef<T>>& b = nullopt,
                const optional<CMatRef<T>>& G = nullopt, const optional<CVecRef<T>>& h = nullopt,
                const optional<CVecRef<T>>& x_lb = nullopt, const optional<CVecRef<T>>& x_ub = nullopt,
        ...
        if (x_lb.has_value()) {
            this->setup_lb_data(x_lb);
        }
        if (x_ub.has_value()) {
            this->setup_ub_data(x_ub);
        }

        this->m_preconditioner.scale_data(this->m_data, reuse_preconditioner,
...
template <typename T, typename I = int, int Mode = KKTMode::KKT_FULL,
          typename Preconditioner = sparse::RuizEquilibration<T, I>>
class SparseSolver
    : public SolverBase<SparseSolver<T, I, Mode, Preconditioner>, T, I, Preconditioner, PIQP_SPARSE, Mode> {
public:
    void setup(const CSparseMatRef<T, I>& P, const CVecRef<T>& c, const optional<CSparseMatRef<T, I>>& A = nullopt,
               const optional<CVecRef<T>>& b = nullopt, const optional<CSparseMatRef<T, I>>& G = nullopt,
               const optional<CVecRef<T>>& h = nullopt, const optional<CVecRef<T>>& x_lb = nullopt,
               const optional<CVecRef<T>>& x_ub = nullopt) {
        this->setup_impl(P, c, A, b, G, h, x_lb, x_ub);
    }

    void update(const optional<CSparseMatRef<T, I>>& P = nullopt, const optional<CVecRef<T>>& c = nullopt,
                const optional<CSparseMatRef<T, I>>& A = nullopt, const optional<CVecRef<T>>& b = nullopt,
                const optional<CSparseMatRef<T, I>>& G = nullopt, const optional<CVecRef<T>>& h = nullopt,
                const optional<CVecRef<T>>& x_lb = nullopt, const optional<CVecRef<T>>& x_ub = nullopt,
                bool reuse_preconditioner = true) {
        if (!this->m_setup_done) {
            piqp_eprint("Solver not setup yet\n");
        ...
        if (P.has_value()) {
            ...
            for (isize j = 0; j < n; j++) {
                ...
                isize P_utri_col_nnz =
                    this->m_data.P_utri.outerIndexPtr()[j + 1] - this->m_data.P_utri.outerIndexPtr()[j];
                if (P_col_nnz < P_utri_col_nnz) {
                    piqp_eprint("P nonzeros missmatch\n");
                    return;
                }
                Eigen::Map<Vec<T>>(this->m_data.P_utri.valuePtr() + this->m_data.P_utri.outerIndexPtr()[j],
                                   P_utri_col_nnz) =
