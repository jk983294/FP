...
struct Settings {
    ...
    T iterative_refinement_static_regularization_rel =
    ...
    bool verify_settings() const noexcept {
        return rho_init > 0 && delta_init > 0 && eps_abs > 0 && eps_rel >= 0 && eps_duality_gap_abs > 0 &&
               eps_duality_gap_rel >= 0 && reg_lower_limit > 0 && reg_finetune_primal_update_threshold >= 0 &&
               reg_finetune_dual_update_threshold >= 0 && max_iter > 0 && max_factor_retires > 0 &&
               preconditioner_iter >= 0 && tau > 0 && tau <= 1 && iterative_refinement_eps_abs > 0 &&
               iterative_refinement_eps_rel >= 0 && iterative_refinement_max_iter >= 0 &&
               iterative_refinement_min_improvement_rate >= 1.0 && iterative_refinement_static_regularization_eps > 0 &&
