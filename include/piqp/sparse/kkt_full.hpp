...
struct KKTImpl<Derived, T, I, KKTMode::KKT_FULL> {
    ...
    SparseMat<T, I> create_kkt_matrix() {
        ...
        for (isize j = 0; j < jj; j++) {
            ...
            isize col_nnz = data.P_utri.outerIndexPtr()[j + 1] - data.P_utri.outerIndexPtr()[j];
            Eigen::Map<Vec<I>>(KKT.innerIndexPtr() + k_kkt, col_nnz) =
                Eigen::Map<const Vec<I>>(data.P_utri.innerIndexPtr() + data.P_utri.outerIndexPtr()[j], col_nnz);
            Eigen::Map<Vec<T>>(KKT.valuePtr() + k_kkt, col_nnz) =
                Eigen::Map<const Vec<T>>(data.P_utri.valuePtr() + data.P_utri.outerIndexPtr()[j], col_nnz);
        ...
        for (isize j = 0; j < jj; j++) {
            ...
            Eigen::Map<Vec<I>>(KKT.innerIndexPtr() + k_kkt, col_nnz) =
                Eigen::Map<const Vec<I>>(data.AT.innerIndexPtr() + data.AT.outerIndexPtr()[j], col_nnz);
            Eigen::Map<Vec<T>>(KKT.valuePtr() + k_kkt, col_nnz) =
                Eigen::Map<const Vec<T>>(data.AT.valuePtr() + data.AT.outerIndexPtr()[j], col_nnz);
        ...
        for (isize j = 0; j < jj; j++) {
            ...
            Eigen::Map<Vec<I>>(KKT.innerIndexPtr() + k_kkt, col_nnz) =
                Eigen::Map<const Vec<I>>(data.GT.innerIndexPtr() + data.GT.outerIndexPtr()[j], col_nnz);
            Eigen::Map<Vec<T>>(KKT.valuePtr() + k_kkt, col_nnz) =
                Eigen::Map<const Vec<T>>(data.GT.valuePtr() + data.GT.outerIndexPtr()[j], col_nnz);
