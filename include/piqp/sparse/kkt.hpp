...
struct KKT : public KKTImpl<KKT<T, I, Mode, Ordering>, T, I, Mode> {
    ...
    void update_scalings(const T& rho, const T& delta, const CVecRef<T>& s, const CVecRef<T>& s_lb,
                         const CVecRef<T>& s_ub, const CVecRef<T>& z, const CVecRef<T>& z_lb, const CVecRef<T>& z_ub) {
        m_rho = rho;
        m_delta = delta;
        m_s = s;
    ...
    void multiply(const CVecRef<T>& delta_x, const CVecRef<T>& delta_y, const CVecRef<T>& delta_z,
                  const CVecRef<T>& delta_z_lb, const CVecRef<T>& delta_z_ub, const CVecRef<T>& delta_s,
                  const CVecRef<T>& delta_s_lb, const CVecRef<T>& delta_s_ub, VecRef<T> rhs_x, VecRef<T> rhs_y,
                  VecRef<T> rhs_z, VecRef<T> rhs_z_lb, VecRef<T> rhs_z_ub, VecRef<T> rhs_s, VecRef<T> rhs_s_lb,
                  VecRef<T> rhs_s_ub) {
        ...
        rhs_s_lb.head(data.n_lb).array() +=
            m_z_lb_inv.head(data.n_lb).array().cwiseInverse() * delta_s_lb.head(data.n_lb).array();

        rhs_s_ub.head(data.n_ub).array() = m_s_ub.head(data.n_ub).array() * delta_z_ub.head(data.n_ub).array();
        rhs_s_ub.head(data.n_ub).array() +=
            m_z_ub_inv.head(data.n_ub).array().cwiseInverse() * delta_s_ub.head(data.n_ub).array();
    }
    ...
    void solve(const CVecRef<T>& rhs_x, const CVecRef<T>& rhs_y, const CVecRef<T>& rhs_z, const CVecRef<T>& rhs_z_lb,
               const CVecRef<T>& rhs_z_ub, const CVecRef<T>& rhs_s, const CVecRef<T>& rhs_s_lb,
               const CVecRef<T>& rhs_s_ub, VecRef<T> delta_x, VecRef<T> delta_y, VecRef<T> delta_z,
               VecRef<T> delta_z_lb, VecRef<T> delta_z_ub, VecRef<T> delta_s, VecRef<T> delta_s_lb,
               VecRef<T> delta_s_ub, bool iterative_refinement) {
        ...
        if (iterative_refinement && settings.iterative_refinement_max_iter > 0) {
            ...
            for (isize i = 0; i < settings.iterative_refinement_max_iter; i++) {
                if (error_norm <=
                    (settings.iterative_refinement_eps_abs + settings.iterative_refinement_eps_rel * rhs_norm)) {
                    break;
                }
                ...
                err_corr_perm.noalias() -=
                    PKPt.transpose().template triangularView<Eigen::StrictlyLower>() * ref_sol_perm;
        ...
        for (isize i = 0; i < data.n_lb; i++) {
            //            delta_z_lb(i) = (-data.x_lb_scaling(i) * delta_x(data.x_lb_idx(i)) - rhs_z_lb(i) +
            //            m_z_lb_inv(i) * rhs_s_lb(i))
            //                            / (m_s_lb(i) * m_z_lb_inv(i) + m_delta);
            delta_z_lb(i) =
        ...
        for (isize i = 0; i < data.n_ub; i++) {
            //            delta_z_ub(i) = (data.x_ub_scaling(i) * delta_x(data.x_ub_idx(i)) - rhs_z_ub(i) +
            //            m_z_ub_inv(i) * rhs_s_ub(i))
            //                            / (m_s_ub(i) * m_z_ub_inv(i) + m_delta);
            delta_z_ub(i) =
        ...
        delta_s_lb.head(data.n_lb).array() =
            m_s_lb.head(data.n_lb).array() * m_z_lb_inv.head(data.n_lb).array() *
            (rhs_s_lb.head(data.n_lb).array() / m_s_lb.head(data.n_lb).array() - delta_z_lb.head(data.n_lb).array());

        delta_s_ub.head(data.n_ub).array() =
            m_s_ub.head(data.n_ub).array() * m_z_ub_inv.head(data.n_ub).array() *
            (rhs_s_ub.head(data.n_ub).array() / m_s_ub.head(data.n_ub).array() - delta_z_ub.head(data.n_ub).array());
        ...
        multiply(delta_x, delta_y, delta_z, delta_z_lb, delta_z_ub, delta_s, delta_s_lb, delta_s_ub, err_x, err_y,
                 err_z, err_z_lb, err_z_ub, err_s, err_s_lb, err_s_ub);
        ...
        std::cout << "kkt_error: " << err_x.template lpNorm<Eigen::Infinity>() << " "
                  << err_y.template lpNorm<Eigen::Infinity>() << " " << err_z.template lpNorm<Eigen::Infinity>() << " "
